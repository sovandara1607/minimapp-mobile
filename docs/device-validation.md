...
Milestone 1 is implemented, but stability requires a real public Mapbox token and native device checks. Follow this on an iPhone and Android phone before starting Milestone 2.

1. Fresh install: grant foreground location. Confirm first position appears, heading follows orientation, and the player sits at approximately 68% of the map viewport height. Confirm Mapbox attribution is accessible.
...
4. Pan, pinch, rotate, and pitch during motion and while recentering. Confirm following stops, gestures remain responsive, and recenter smoothly restores bearing, pitch, zoom, and padding. Toggle 2D/3D in both follow and explore. Face north and recenter again.
...
- 11 deterministic tests for motion, camera geometry, simulation, and style schema.
